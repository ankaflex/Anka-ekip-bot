...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    let isim = args[1][0].toUpperCase() + args[1].substring(1);
        ...
        await message.guild.member(target.id).setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${isim} | ${yaş}`).then(x => message.react(client.emojis.cache.find(x => x.name === "ramal_tik")))
