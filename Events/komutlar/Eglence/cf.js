	...
	if (sec == "all") {
        ...
        return message.channel.send(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "ramal_coin")}`).then(async mesaj => setTimeout(async () => {
            if (durums === "Tebrikler Kazandın") {
                ...
                mesaj.edit(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "ramal_coin")} ve 2 katını kazandın!**`)
            } else {
                mesaj.edit(`**${message.author.username}**, **${data.para <= 50000 ? data.para : 50000}** :dollar: para harcadı.\nMadeni para dönüyor... ${client.emojis.cache.find(x => x.name === "ramal_coin")} tüm paranı kaybettin... :c`)
            }
        }, 5000))
	}
